...
int main()
{
  ...
  REP(a,temp.size())
  {
  	...
  	{
  		if(stat == 0) jam += temp[a];
  		else menit += temp[a];
  	}
  }
  
  int jamx,menitx;
  jamx = StringToInt(jam);
  menitx = StringToInt(menit);
  ...
  REP(a,N)
  {
  	...
  	if(a == 0) KPK = val;
  	else KPK = lcm(KPK,val);
  }
  
  //printf("KPK : %d\n",KPK);
  jamx = jamx + (KPK / 60);
  menitx = menitx + (KPK % 60);
  ...
  if(jamx >= 24) jamx = jamx % 24;
  
  if(jamx < 10) printf("0");
  printf("%d.",jamx);
  
  if(menitx < 10) printf("0");
  printf("%d\n",menitx);
